...
class User(Base):
    ...
    institution_id: Mapped[str] = mapped_column(
        String, ForeignKey("institutions.id", ondelete="CASCADE"), nullable=True, index=True
    )
    name: Mapped[str] = mapped_column(String, nullable=True)
    email: Mapped[str] = mapped_column(String, unique=True, index=True, nullable=True)
    role: Mapped[str] = mapped_column(String, default="student", index=True)  # admin, coordinator, faculty, student
...
class Department(Base):
    ...
    institution_id: Mapped[str] = mapped_column(
        String, ForeignKey("institutions.id", ondelete="CASCADE"), index=True
    )
    name: Mapped[str] = mapped_column(String, index=True)
...
class Course(Base):
    ...
    department_id: Mapped[str] = mapped_column(
        String, ForeignKey("departments.id", ondelete="CASCADE"), index=True, nullable=True
    )
    code: Mapped[str] = mapped_column(String, index=True)
    name: Mapped[str] = mapped_column(String, index=True)
    description: Mapped[str] = mapped_column(Text, nullable=True)
...
class Enrollment(Base):
    ...
    status: Mapped[str] = mapped_column(String, default="active")  # active, completed, dropped
...
class Timetable(Base):
    ...
    day_of_week: Mapped[str] = mapped_column(String, index=True)  # Monday, Tuesday, etc.
    start_time: Mapped[str] = mapped_column(String)  # "09:00"
    end_time: Mapped[str] = mapped_column(String)  # "10:30"
...
class Attendance(Base):
    ...
    status: Mapped[str] = mapped_column(String, default="present")  # present, absent, late, excused
    marked_by: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class Submission(Base):
    ...
    assignment_id: Mapped[str] = mapped_column(
        String, ForeignKey("assignments.id", ondelete="CASCADE"), index=True
    )
    student_id: Mapped[str] = mapped_column(
        String, ForeignKey("users.id", ondelete="CASCADE"), index=True
    )
    ...
    ai_grade_draft: Mapped[dict] = mapped_column(JSON, nullable=True)
    approved_by: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class Notice(Base):
    ...
    title: Mapped[str] = mapped_column(String, index=True)
    content: Mapped[str] = mapped_column(Text)
    target_role: Mapped[str] = mapped_column(String, default="all")  # all, faculty, student
    created_by: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class AIWorkflowTask(Base):
    __tablename__ = "ai_workflow_tasks"
    id: Mapped[str] = mapped_column(
        String, primary_key=True, default=lambda: str(uuid4())
    )
    institution_id: Mapped[str] = mapped_column(
    ...
    workflow_type: Mapped[str] = mapped_column(String, index=True)  # grade_recommendation, attendance_flag, notice_draft
    payload: Mapped[dict] = mapped_column(JSON)
    status: Mapped[str] = mapped_column(String, default="pending_approval", index=True)  # pending_approval, approved, rejected
    created_by: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    approved_by: Mapped[str] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
