        ...
        with self._get_db_session() as db:
            try:
                ...
                if user:
                    ...
                    fallback_name = (email.split("@")[0] if email else f"user_{user_id[:8]}")
                    new_user = User(
                        id=user_id,
                        email=email,
                ...
                db.rollback()
                # Fetch user created concurrently
                existing = db.query(User).filter(
                    (User.id == user_id) | (User.email == email) if email else (User.id == user_id)
                ).first()
                if existing:
